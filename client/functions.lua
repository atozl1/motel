...
Init = function()
    ESX.TriggerServerCallback("motel:fetchMotels", function(fetchedMotels, fetchedStorages, fetchedName)
...
OpenMotelRoomMenu = function(motelRoom)
    ...
    if cachedMotelRoom then
        for roomIndex, roomData in ipairs(cachedMotelRoom["rooms"]) do
            ...
            table.insert(menuElements, {
                ["label"] = allowed and "Enter " .. roomData["displayLabel"] .. "'nin odası" or roomData["displayLabel"] .. "'s oda kilitli, kapıyı çalabilirsin.",
                ["action"] = roomData,
                ["allowed"] = allowed
            })
        end
    ...
    if #menuElements == 0 then
        table.insert(menuElements, {
            ["label"] = "Odalar resepsiyondan alınıyor."
        })
    end
...
OpenInviteMenu = function(motelRoomData)
    ...
    if #closestPlayers <= 0 then
        return ESX.ShowNotification("Kapının önünde kimse yok.")
    end
    ...
    ESX.TriggerServerCallback("motel:retreivePlayers", function(playersRetreived)
        if playersRetreived then
            ...
            ESX.ShowNotification("Oyuncuların bilgileri alınamadı.", "error", 3500)
        end
    end, closestPlayers)
end
...
OpenLandLord = function()
    if GetPlayerMotel() then return ESX.ShowNotification("You own a room.") end
...
OpenConfirmBox = function(motelRoom)
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "main_accept_motel", {
        ["title"] = "Kiralamayı onaylıyor musunuz #" .. motelRoom .. "?",
        ["align"] = Config.AlignMenu,
        ["elements"] = {
            {
                ["label"] = "Evet.",
        ...
        if action == "yes" then
            ESX.TriggerServerCallback("motel:buyMotel", function(bought)
                if bought then
                    ESX.ShowNotification("Oda kiralandı #" .. motelRoom)
		...
		ESX.UI.Menu.Open("default", GetCurrentResourceName(), "motel_main_dressing_menu", {
			...
			TriggerEvent("skinchanger:getSkin", function(skin)
                ESX.TriggerServerCallback("motel:getPlayerOutfit", function(clothes)
                    ...
                    ESX.ShowNotification("Kıyafetini giydin.")
                end, currentOutfit)
			end)
        end, function(menuData, menuHandle)
			menuHandle.close()
    ...
    if insideMotel then
        ...
        if ownedMotel then
            ...
            ESX.Game.Teleport(PlayerPedId(), Config.LandLord["position"], function()
                ESX.ShowNotification("Odada çıkış yaptığınız için geri getirildiniz.")
            end)
        end
